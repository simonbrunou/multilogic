...
### Greco-Latin result — honest construction rating (2026-06-15)

**Different in kind:** Greco-Latin is a **construction** puzzle (no unique solution, no deduction techniques), so it does NOT use the technique-rating framework. It rates by the **residual free-choice ratio**: after propagating forced singles (empty cells with exactly one consistent (a,b) pair), the fraction of originally-empty cells that still need a free guess. 0 = fully deducible (trivial); 1 = pure guessing. This is the empirically-validated human-effort proxy (a "fully forced" puzzle is easy for a human regardless of how much a naive backtracker thrashes — so the old backtracking-effort gradient was a machine artifact). Shared `analyze`/`candidatesAt` extracted into `grecolatin/candidates.ts`. The previous `achievedDifficulty` was just the echoed request (dishonest — a sparsely-revealed-but-forced "expert" played easy); it is now the real rating, and generation **floors** by re-rolling reveals until the rated band matches the request (else closest).

**Investigation finding (parallels Kakuro):** at the fixed order **n=5** the residual ratio is **bimodal/cliff-like** — reveal ≥~0.4 ⇒ ratio ~0 (fully forced = easy), reveal ≤~0.3 ⇒ ratio ~0.94–1.0 (guessing = hard/expert), sharp transition ~0.37. Calibrated cuts 0.05 / 0.47 / 0.97. So `easy`/`hard`/`expert` are real and reachable but **`medium` is squeezed** (barely exists at n=5). **Stayed at n=5** — raising the order would widen the range but the `hint.ts` backtracker can freeze the UI at n≥7 (needs MRV/timeout first; out of scope).
