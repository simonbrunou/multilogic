...
function analyze(n: number, grid: number[]): Analysis {
  const mk = () => Array.from({ length: n }, () => new Set<number>());
  const a = { aRow: mk(), aCol: mk(), bRow: mk(), bCol: mk(), pairs: new Set<number>() };
  for (let i = 0; i < n * n; i++) {
...
function candidatesAt(n: number, an: Analysis, i: number): { a: number; b: number }[] {
  const r = Math.floor(i / n), c = i % n;
