...
/**
 * How many column totals to hide per difficulty. One hidden total is exactly
 * recoverable (`Σ d² − shown`), costing the player a single deduction; hiding
 * two leaves only their *joint* sum known and the split between them ambiguous,
 * removing a whole column-sum anchor and forcing much more case analysis. Expert
 * hides two for a markedly harder grid (see the generator/rater calibration
 * tests); the easier bands hide one.
 */
const HIDDEN: Record<Difficulty, number> = { easy: 1, medium: 1, hard: 1, expert: 2 };
 ...
 *
 * Build a known-valid solution, hide `HIDDEN[target]` column totals (the player
 * must deduce those columns' sums, which makes the grid harder), then seed every
 * interior cell and dig: remove a seed only when the result stays uniquely
 * solvable AND does not rate harder than the target (so more-seeds-is-easier
 * holds and difficulty stays bounded). The hidden totals' joint sum is fixed by
 * `Σ d²` (see `totalCaps`/`totalsSatisfied`), so the uniqueness oracle and rater
 * reason about them as a joint constraint and the final instance is always
...
export function generateForDifficulty(prng: PRNG, target: Difficulty): GeneratedYakuso {
  ...
  const nHidden = HIDDEN[target];

  for (let attempt = 0; attempt < 50; attempt++) {
    const solution = randomSolution(prng, rows, cols);
    // Hide `nHidden` column totals from the player; the rest stay shown. The
    // single-hidden path keeps one `prng.int` draw so easy/medium/hard seeds
    // reproduce exactly as before; only expert (two hidden) shifts the stream.
    const hidden = new Set(
      nHidden === 1
        ? [prng.int(cols)]
        : prng.shuffle(Array.from({ length: cols }, (_, c) => c)).slice(0, nHidden)
    );
    const totals: (number | null)[] = columnSums(solution, rows, cols).map((t, c) => (hidden.has(c) ? null : t));
