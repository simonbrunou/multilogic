 ...
 *
 * Build a known-valid solution, seed every interior cell, then dig: remove a
 * seed only when the result stays uniquely solvable AND does not rate harder
 * than the target (so more-seeds-is-easier holds and difficulty stays bounded).
 * The uniqueness oracle runs on every removal; the final instance is always
...
export function generateForDifficulty(prng: PRNG, target: Difficulty): GeneratedYakuso {
  ...
  for (let attempt = 0; attempt < 50; attempt++) {
    ...
    const totals = columnSums(solution, rows, cols);
